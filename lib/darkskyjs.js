    ...
    (function (global, factory) {
        ...
        */
        ...
        function DarkSky( config ) {
            ...
            this.url = ( typeof config.PROXY_SCRIPT !== 'undefined' ) ? config.PROXY_SCRIPT + 'https://api.darksky.net/forecast/'+ config.API_KEY + '/' : 'https://api.darksky.net/forecast/' + config.API_KEY + '/';
        }
        ...
        DarkSky.prototype.getURL = function getURL( latitude, longitude ) {
            ...
            var optional = "?";
            optional += this.LANG?"lang="+this.LANG:"";
            optional+= optional!="?"?"&":"";
            optional +=this.UNITS?"units="+this.UNITS:"";

            if(optional=="?")
                optional = "";

            console.log(this.url + latitude + ',' + longitude+optional);
    
            return this.url + latitude + ',' + longitude+optional;
        }
        ...
        DarkSky.prototype.requestData = function requestData( latitude, longitude ) {
            ...
            var res = this.makeRequest( 'GET', requestUrl );
            return res;
        };
        ...
        DarkSky.prototype.processHourly = function processHourly(parsedData, locations, i) {
            return new DarkSkyConditions( parsedData.hourly, locations[ i ].name );
        }
    
        DarkSky.prototype.processAlerts = function processAlerts(parsedData, locations, i) {
    
            return new DarkSkyConditions( parsedData.alerts, locations[ i ].name );
        }
        ...
        DarkSky.prototype.getHourlyConditions = function getHourlyConditions( locations, appCallback ) {
            this.initAllReqs(locations, 'processHourly', appCallback);
        };
    
        DarkSky.prototype.getAlerts = function getAlerts( locations, appCallback ) {
            this.initAllReqs(locations, 'processAlerts', appCallback);
        };
